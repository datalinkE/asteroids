...
RawImageData::RawImageData(const void* png_data, const int png_data_size)
{
	DLOG() << ARG(png_data_size);
    assert(png_data != NULL && png_data_size > 8);
    assert(png_check_sig(static_cast<const png_byte*>(png_data), 8));
    ...
    ReadDataHandle png_data_handle = (ReadDataHandle) {{static_cast<const png_byte*>(png_data), png_data_size}, 0};
    ...
    size = raw_image.size;
    gl_color_format = get_gl_color_format(png_info.color_type);
    data = raw_image.data;
}
...
static PngInfo read_and_update_info(const png_structp png_ptr, const png_infop info_ptr)
{
    ...
    DLOG() << ARG(width) << ARG(height) << ARG(color_type);
    ...
    const png_uint_32 height)
{
    ...
    png_byte* raw_image = static_cast<png_byte*>( new unsigned char[data_length] );
...
static GLenum get_gl_color_format(const int png_color_format) {
    ...
    switch (png_color_format) {
        case PNG_COLOR_TYPE_GRAY:
        	DLOG() << "PNG_COLOR_TYPE_GRAY -> GL_LUMINANCE";
            return GL_LUMINANCE;
        case PNG_COLOR_TYPE_RGB_ALPHA:
        	DLOG() << "PNG_COLOR_TYPE_ALPHA -> GL_RGBA";
            return GL_RGBA;
        case PNG_COLOR_TYPE_GRAY_ALPHA:
        	DLOG() << "PNG_COLOR_TYPE_GRAY_ALPHA -> GL_LUMINANCE_ALPHA";
